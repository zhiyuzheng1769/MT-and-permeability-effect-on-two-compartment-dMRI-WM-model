...
function main()
    ...
    geom = MCMRSimulator.read_geometry("distributed_cylinders_samevar/permeability/cylinders_MT_0_sus_0_perm_"*@sprintf("%.3f",perm)*"_rmean_"*@sprintf("%.2f",radius)*"_density_0.65.json")
    sim = Simulation(seqs, diffusivity=3., geometry=geom)
    sig = readout(160000, sim, bounding_box=BoundingBox([-sz,-sz,-sz],[sz,sz,sz]), subset=[Subset(inside=true), Subset(inside=false), Subset()])
